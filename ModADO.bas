...
AccessCnn = "DRIVER={Microsoft Access Driver (*.mdb)};" & "DBQ=db1.mdb;" & "DefaultDir=" & App.Path + "\" & ";" & "UID=admin;PWD=;"

 StringCnn = "Provider=Microsoft.Jet.OLEDB.4.0; Data Source=" & App.Path & "\db1.mdb;Persist Security Info=False;"


Conn1.ConnectionString = AccessCnn
Conn1.Open

Rs1.Open "SELECT * FROM [table]", StringCnn, adOpenStatic, adLockBatchOptimistic, adCmdText
End Function
